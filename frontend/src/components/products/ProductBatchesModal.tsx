...
export default function ProductBatchesModal({
  ...
  return (
    <Dialog
      ...
      onClose={onClose}
      title={<>Партии товара (FIFO): {selectedProduct?.name}</>}
      icon={<Layers size={18} />}
      widthClassName="max-w-[58rem]"
      footer={
        <button type="button" onClick={onClose} className="app-button-secondary min-w-[120px]">
          Закрыть
        </button>
      }
    >
            ...
            <div key={b.id} className={clsx('rounded-3xl border border-slate-100 p-4', i === 0 ? 'bg-violet-50/60' : 'bg-slate-50')}>
              ...
              <div className="mt-4 grid grid-cols-2 gap-3">
                <div className="rounded-2xl bg-white px-3 py-3">
                  ...
                  {quantityInfo.secondary && <p className="mt-1 text-[11px] font-medium text-slate-500">{quantityInfo.secondary}</p>}
                </div>
                <div className="rounded-2xl bg-white px-3 py-3">
                  ...
                  {remainingInfo.secondary && <p className="mt-1 text-[11px] font-medium text-slate-500">{remainingInfo.secondary}</p>}
                </div>
              </div>
            ...
            return (
              <tr key={b.id} className={clsx('text-[13px]', i === 0 && 'bg-violet-50/40')}>
                ...
                <td className="py-3 text-right font-bold text-slate-400">
                  ...
                  {quantityInfo.secondary && <div className="mt-1 text-[11px] font-medium text-slate-400">{quantityInfo.secondary}</div>}
                </td>
                <td className="py-3 text-right font-black text-slate-900">
                  <div className="whitespace-pre-line">{remainingInfo.primary}</div>
                  {remainingInfo.secondary && <div className="mt-1 text-[11px] font-medium text-slate-400">{remainingInfo.secondary}</div>}
                </td>
