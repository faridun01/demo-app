...
router.get('/:id/history', async (req: AuthRequest, res, next) => {
  try {
    ...
    const cp1252ByteByCodePoint = new Map<number, number>([
    ...
    const decodeMojibake = (value: string) => {
      ...
      if (!source.includes(String.fromCharCode(0x00d0)) && !source.includes(String.fromCharCode(0x00d1))) {
        return source;
      }

      try {
        const bytes = Array.from(source).flatMap((char) => {
          const codePoint = char.codePointAt(0) || 0;
          if (codePoint <= 0xff) {
            return [codePoint];
          }
          const cp1252Byte = cp1252ByteByCodePoint.get(codePoint);
          return cp1252Byte === undefined ? Array.from(Buffer.from(char, 'utf8')) : [cp1252Byte];
        });
